...
# ── Живой сбой 2026-07-23: send_morning_digest НЕ доставлен (telegram.error.
# BadRequest "Can't parse entities" на НОВОМ поле -- event-заголовок биржи --
# не покрытом фиксом 2026-07-14 выше). Владелец, ДА: общий санитайзер +
# обязательный фолбэк на голый текст. ──

def test_sanitize_markdown_free_text_neutralizes_special_chars():
    raw = "wash_class *risk* `code` [bracket]"
    out = morning_metrics.sanitize_markdown_free_text(raw)
    assert "_" not in out and "*" not in out and "`" not in out
    assert "[" not in out and "]" not in out


def test_sanitize_markdown_free_text_honest_noop_on_empty_or_none():
    assert morning_metrics.sanitize_markdown_free_text("") == ""
    assert morning_metrics.sanitize_markdown_free_text(None) is None


def test_build_morning_digest_top_discrepancy_detail_is_sanitized(monkeypatch, tmp_path):
    """Регресс: top_discrepancy.detail -- свободный текст из shadow-записи
    (discrepancy[0], см. daily_metrics._format_top_discrepancy) -- может в
    принципе содержать непарные Markdown-символы (например, интерполированное
    сообщение исключения) -- должен пройти через sanitize_markdown_free_text()
    перед попаданием в дайджест."""
    _patch_common(monkeypatch, tmp_path)
    shadow_file = tmp_path / "shadow_signals.json"
    shadow_file.write_text(
        '{"records": [{"symbol": "TESTUSDT", "direction": "long", "ts": 999999.0, '
        '"promoted_live": true, "patches_affected": ["01"], '
        '"discrepancy": ["wash_class *flag* triggered `here`"]}]}'
    )
    monkeypatch.setattr(daily_metrics, "shadow_engine_file", lambda: str(shadow_file))
    text = morning_metrics.build_morning_digest(_FakeBotModule(), now_ts=1_000_000.0)
    discrepancy_line = next(l for l in text.split("\n") if "Топ-1 расхождение" in l)
    assert "_" not in discrepancy_line
    assert "*" not in discrepancy_line
    assert "`" not in discrepancy_line
    assert "wash class" in discrepancy_line  # содержание сохранено


# ── send_morning_digest() -- обязательный фолбэк на голый текст ──

class _FakeBot:
    def __init__(self, fail_on_markdown=False):
        self.fail_on_markdown = fail_on_markdown
        self.calls = []
    ...
    async def send_message(self, chat_id, text, parse_mode=None):
        ...
        if self.fail_on_markdown and parse_mode == "Markdown":
            raise BadRequest("Can't parse entities: can't find end of the entity starting at byte offset 123")


def test_send_morning_digest_sends_markdown_when_no_error(monkeypatch, tmp_path):
    _patch_common(monkeypatch, tmp_path)
    monkeypatch.setattr(morning_metrics, "build_morning_digest", lambda bot_module: "digest text")
    fake_bot = _FakeBot(fail_on_markdown=False)
    asyncio.run(morning_metrics.send_morning_digest(fake_bot, owner_id=42))
    assert len(fake_bot.calls) == 1
    assert fake_bot.calls[0]["parse_mode"] == "Markdown"
    assert fake_bot.calls[0]["text"] == "digest text"


def test_send_morning_digest_falls_back_to_plain_text_on_entity_parse_error(monkeypatch, tmp_path):
    """Регресс на живой сбой 2026-07-23: если Markdown-отправка падает на
    "Can't parse entities" (ЛЮБАЯ причина, известная или будущая) -- дайджест
    ОБЯЗАН всё равно дойти голым текстом, не пропасть молча."""
    _patch_common(monkeypatch, tmp_path)
    monkeypatch.setattr(morning_metrics, "build_morning_digest", lambda bot_module: "digest text")
    fake_bot = _FakeBot(fail_on_markdown=True)
    asyncio.run(morning_metrics.send_morning_digest(fake_bot, owner_id=42))
    assert len(fake_bot.calls) == 2
    assert fake_bot.calls[0]["parse_mode"] == "Markdown"
    assert fake_bot.calls[1]["parse_mode"] is None
    assert fake_bot.calls[1]["text"] == "digest text"  # то же содержание, дошло


def test_send_morning_digest_other_bad_request_does_not_trigger_fallback(monkeypatch, tmp_path):
    """Честная граница: фолбэк -- ТОЛЬКО для "can't parse entities", не для
    ЛЮБОЙ ошибки Telegram (например, chat not found -- фолбэк тут бессмыслен,
    голый текст в тот же несуществующий chat_id тоже не дойдёт)."""
    _patch_common(monkeypatch, tmp_path)
    monkeypatch.setattr(morning_metrics, "build_morning_digest", lambda bot_module: "digest text")
    ...
    fake_bot = _ChatNotFoundBot()
    asyncio.run(morning_metrics.send_morning_digest(fake_bot, owner_id=42))
    assert len(fake_bot.calls) == 1  # НЕ повторил -- другой класс ошибки
