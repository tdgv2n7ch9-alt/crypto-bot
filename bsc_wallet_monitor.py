...
# Владелец, ДА, 2026-07-24 (сужение скоупа до "вариант 1" -- живой режим,
# устойчивый по QuickNode-бюджету): watch-list сужен до AKE топ-кластера
# (BANK-кластер НЕ добавлен -- владелец явно оставил BANK закрытым/архивным
# по решению 2026-07-19, см. knowledge/METHODOLOGY_CORE.md §22). Убраны:
# обе "прокладки" (0x561beec8.../0x0a960739...) -- однодневные адреса,
# слившие всё через DEX ещё 2026-07-16, наблюдение за ними больше не даёт
# новой информации (тот же принцип, что закрытие BANK-кейса).
AKE_WATCHED_WALLETS = [
    "0x27333Bd8c321a263B0565e69eea3b736b9d1f42c",  # #1, 18.3671% supply на момент фиксации
    # 2026-07-15; ИСПРАВЛЕНО 2026-07-24: баланс НИКОГДА не менялся с 07-15 (9 точек
    # eth_call balanceOf(), 2 независимых RPC-провайдера, идентичный результат до 18
    # знака) -- прошлая запись "продан 2026-07-16" была ошибкой WebFetch-пересказа
    # страницы BscScan, не прямым RPC-вызовом, см. knowledge/AKE_DOSSIER.md
    "0xD229b65d50E412cC3C394233E7a53A1DAc4dA457",  # #2, 15.0000% supply -- тот же
    # честный фикс: баланс никогда не менялся, см. knowledge/AKE_DOSSIER.md
...
AKE_WATCHED_RECIPIENTS = [
    "0x4a7063961ad7b449d607bc51d6000c290f2165df",  # владелец, ДА, 2026-07-24: BscScan-тег
    # "Gate: Deposit Funder" (Gate.io депозитный адрес) -- подтверждённый получатель
    # разгрузки AKE от 0x73d8 (см. knowledge/AKE_DOSSIER.md). Следим как за получателем
    # (topics[2], НЕ topics[1] -- см. _recipient_topics()) на предмет повторных волн.
]
...
ALERT_THRESHOLD_USD = 50_000  # владелец, ДА, 2026-07-24: понижено с $200K -- "любой
# исходящий >$50K с watch-list кошелька -> алерт" (сужение скоупа компенсирует
# более чувствительный порог, не разгоняет частоту алертов сверх разумного)

# Владелец, 2026-07-16 (приёмка задачи #2, п.1): детект "прокладка сливает" --
# если watched-кошелёк переводит AKE НА адрес известного DEX-роутера, это сам
# по себе значимый сигнал (начало ликвидации) независимо от суммы, отдельно
# от общего ALERT_THRESHOLD_USD ($200K может не набраться на одном чанке
# многоступенчатого слива). Адреса -- НЕ из общей документации "известных
# роутеров", а проверены живьём по факту (BscScan, 2026-07-16): обе
# прокладки (#1 0x561beec8..., #2 0x0a960739...) в своих реальных "Dag Swap
# By Order" транзакциях за 15-16.07 шли ИМЕННО на этот адрес -- подтверждено
# и на странице самих прокладок (txlist), и на странице самого роутера
# (публичный тег "OKX: DEX Router 2"), независимыми запросами.
# PancakeSwap V2 Router02 добавлен как официально документированный адрес
# (PancakeSwap developer docs + BscScan, https://developer.pancakeswap.finance/
# contracts/v2/addresses) для покрытия -- на прокладках живьём НЕ наблюдался,
# честно отмечено в комментарии, не выдаётся за подтверждённый факт по этому
# кейсу.
DEX_ROUTER_ADDRESSES = {
    "0x62ccef0b4545166f721caa9fee13c1d3767e27dc": "OKX: DEX Router 2",  # подтверждено
    # живьём -- обе прокладки реально сливали AKE через этот адрес, BscScan 2026-07-16
    "0x10ed43c718714eb63d5aa57b78b54704e256024e": "PancakeSwap: Router v2",  # офиц.
    # документированный адрес, на прокладках живьём не наблюдался
}
...
# Владелец, ДА, 2026-07-24 (сужение скоупа, "вариант 1" -- живой режим): 1
# скан/час вместо 1/мин (POLL_INTERVAL_SEC 60 -> 3600) -- ЭТО ТРЕБУЕТ
# пропорционально увеличить MAX_BLOCKS_PER_TICK, иначе монитор мгновенно
# отстаёт от цепи (BSC ~3с/блок -> ~1200 блоков/час; со старым MAX_BLOCKS_
# PER_TICK=50 на часовом интервале монитор отставал бы на ~1150 блоков
# КАЖДЫЙ час -- ровно тот класс регресса, что уже нашли живьём в
# knowledge/AKE_DOSSIER.md, 2026-07-23: ~42 дня/1.2M блоков отставания).
# 1200 посчитано под сам бюджет-потолок ниже: при max_range=5 (QuickNode)
# и 2 отдельных запросах за тик (sender+recipient topics, см. check_ake_
# wallets) -- 1200/5*2=480 вызовов/тик * 20 credits * 24 тика/сутки * 30
# = ~6.91M credits/мес, под MONTHLY_CREDIT_BUDGET=8M с запасом ~13%.
...
MAX_BLOCKS_PER_TICK = 1200  # см. расчёт бюджета выше -- покрывает ~1 час блоков BSC
REDUCED_MAX_BLOCKS_PER_TICK = 300  # throttled-режим (бюджет над потолком) -- та же
# пропорция уменьшения (~4x), что была у старой пары 50/20, применённая к новому
# часовому масштабу (1200/300=4, было 50/20=2.5) -- покрывает ~15 мин/час вместо
# полного часа, но не превращает throttle в полную остановку мониторинга
...
def _atomic_write_json(path: str, obj) -> bool:
    ...
    Владелец, живая находка 2026-07-17: `{path}.tmp{os.getpid()}` коллизирует,
    когда ДВА разных монитора в ОДНОМ процессе (bsc_wallet_monitor и onchain_watch,
    оба используют этот же _rpc_call/_record_quicknode_call через
    run_in_executor) пишут в один и тот же QUICKNODE_CALL_LOG_FILE конкурентно --
    оба потока получают ОДИНАКОВЫЙ os.getpid() (один процесс, разные executor-
    потоки), тот же tmp-путь, гонка на os.replace ("No such file or directory",
    один поток уже переименовал/удалил файл другого). Добавлен threading.get_ident()
    -- уникален per-thread, коллизия больше невозможна даже при конкурентных
    вызовах из разных мониторов."""
    tmp_path = f"{path}.tmp{os.getpid()}_{threading.get_ident()}"
    try:
...
def format_dex_drain_alert_text(event: dict, router_name: str) -> str:
    """Владелец, 2026-07-16 (приёмка задачи #2, п.1): watched-кошелёк
    переводит AKE НА адрес известного DEX-роутера (DEX_ROUTER_ADDRESSES) --
    сам факт значим (начало ликвидации через своп), независимо от суммы."""
    return (
        f"🚨 ПРОКЛАДКА СЛИВАЕТ\n"
        f"{event['from']} -> {router_name} ({event['to']}): "
        f"{event['amount']:,.0f} AKE"
...
async def check_ake_wallets(bot, send_system_fn=None, run_in_executor_fn=None) -> int:
    ...
    # Владелец, ДА, 2026-07-24 (бюджет-гард с логом остатка): при часовом
    # интервале лог раз в тик -- разумная частота (не спам, как было бы на
    # интервале 1 мин). Честное "н/д" в первые 5 мин жизни процесса --
    # quicknode_budget_report() сам это возвращает, не выдумываем цифру.
    budget = quicknode_budget_report()
    if budget["ok"]:
        log.info(f"bsc_wallet_monitor: бюджет QuickNode -- прогноз "
                  f"{budget['credits_per_month_projected']:,.0f}/{MONTHLY_CREDIT_BUDGET:,.0f} "
                  f"credits/мес ({budget['credits_per_month_projected'] / MONTHLY_CREDIT_BUDGET * 100:.0f}%), "
                  f"throttled={budget['throttled']}")
    else:
        log.info(f"bsc_wallet_monitor: бюджет QuickNode -- {budget['reason']}")
    ...
    for lg in logs + recipient_logs:
        ...
        dex_router_name = DEX_ROUTER_ADDRESSES.get(ev["to"])
        ...
        elif dex_router_name is not None:
            # Владелец, 2026-07-16 (приёмка задачи #2): watched-кошелёк слил AKE
            # в известный DEX-роутер -- значимо само по себе (начало ликвидации),
            # без порога по сумме -- многоступенчатый слив может не набрать
            # ALERT_THRESHOLD_USD на одном чанке.
            try:
                await send_system_fn(bot, format_dex_drain_alert_text(ev, dex_router_name), critical=True)
            except Exception as e:
                log.error(f"bsc_wallet_monitor: send_system (dex-drain) failed: {e}")
