...
(владелец/аналитик размечает зоны на 4h — НЕ вычисляется ботом; см. правило
владельца 2026-07-18 "источники не упоминаются нигде" -- `source`/`note` из
watch_zones.json НИКОГДА не рендерятся с именами в подписчицкий текст,
см. format_level_alert()).
...
ежедневно с телефона через /zones_set, без деплоя. Формат:
  {"updated": "YYYY-MM-DD", "source": "tier_a",
...
def format_level_alert(symbol: str, zone: dict, price: float, state: str,
    ...
    `updated` приходят из watch_zones.json (owner-редактируемый текст через
    /zones_set), поэтому экранируются html.escape() перед вставкой, чтобы случайный
    `<`/`>`/`&` в заметке не ломал разметку сообщения.

    `source` НИКОГДА не рендерится в подписчицкий текст (владелец, 2026-07-18,
    правило "источники не упоминаются нигде" -- см. `tests/
    test_level_watch_no_source_attribution.py`) -- параметр принимается для
    обратной совместимости вызывающей стороны/тестов, но сознательно
    игнорируется здесь. Показывается только `updated` (дата разметки)."""
    side = zone["side"]
    lo, hi = zone["lo"], zone["hi"]
    ...
    src_line = html.escape(updated) if updated else "?"
