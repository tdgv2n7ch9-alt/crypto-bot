...
async def run_signal_loop(bot, tg_bot, owner_chat_id):
    ...
    Владелец, #285-b продолжение (2026-07-19, живая находка ПОСЛЕ фикса check_alerts):
    _stage1_screen()/_stage2_check() -- синхронные, сетевые (bot._fetch_coingecko_oi_map(),
    bot.get_all_coins(), fa_engine.build_full_analysis() -> _cg_get()) -- на CoinGecko
    429 их circuit breaker (_cg_get) синхронно спит до 30-60+с ПРЯМО В EVENT LOOP, тот же
    класс регресса, что и check_alerts/send_scheduled, только эта функция (единственный
    оставшийся незавёрнутый вызов get_all_coins()/-derived, см. bot.py) была пропущена --
    подтверждено логами: bsc_wallet_monitor тик-гэпы 217с/171с и Pump Radar (coarse)
    ...
    for cand in candidates:
        if _daily_cap_reached():
            ...
            break
        symbol = cand["symbol"]
        result = await loop.run_in_executor(None, _stage2_check, symbol, cand["coin"])
        if not result:
