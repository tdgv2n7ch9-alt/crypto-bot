...
BOT_VERSION = "v132"         # обновлять при каждом коммите с изменением bot.py
...
# Владелец, приёмка v130 (2026-07-16): живая находка -- даже хорошо разнесённые
# по времени вызовы (~10.5с между ними, заметно выше _CG_MIN_INTERVAL=2.5с)
# ловили 429 КАЖДЫЙ подряд (лог refresh_watch_zones_rug_cache: SOL/GRAM/LAB/AWE/
# ADA/AVAX/JASMY/BTC/ONDO/WLD/AKE -- все 11 отказали). Один фиксированный ретрай
# (_CG_429_RETRY_WAIT) не помогает при УСТОЙЧИВОМ ограничении (не разовый всплеск,
# а окно, в течение которого источник стабильно недоступен) -- продолжать долбить
# сеть при известном текущем бане только тратит время и приближает более долгий
# бан. Циркуит-брейкер: после серии подряд идущих 429 -- пауза БЕЗ сетевых вызовов
# на экспоненциально растущее окно, сброс счётчика на первом успехе.
_cg_consecutive_429 = 0
_cg_cooldown_until = 0.0
_CG_COOLDOWN_BASE_SEC = 30
_CG_COOLDOWN_MAX_SEC = 300


def cg_rate_limit_status() -> dict:
    """Для /health и /startup-отчёта -- текущее состояние circuit breaker'а
    CoinGecko, без побочных эффектов."""
    now = time.time()
    in_cooldown = now < _cg_cooldown_until
    return {
        "in_cooldown": in_cooldown,
        "cooldown_remaining_sec": max(0.0, _cg_cooldown_until - now) if in_cooldown else 0.0,
        "consecutive_429": _cg_consecutive_429,
    }
...
def _cg_get(url: str, params: dict = None, timeout: int = 10):
    ...
    комментарий у _CG_MIN_INTERVAL), не бесконечный ретрай. Плюс circuit breaker
    (владелец, приёмка v130, 2026-07-16) -- см. комментарий у _cg_consecutive_429."""
    global _cg_last_call_ts, _cg_consecutive_429, _cg_cooldown_until
    params = params or {}
    ...
    with _cg_lock:
        if r.status_code == 429:
            ...
            _cg_cooldown_until = time.time() + cooldown
            log.info(f"_cg_get: 429 после ретрая, подряд {_cg_consecutive_429}-й раз -- "
                     f"circuit breaker на {cooldown:.0f}с")
        else:
            _cg_consecutive_429 = 0
    r.raise_for_status()
    data = r.json()
...
# Whale Radar (Блок 1/2, whale_radar.py -- on-chain/стакан) -- Пакет 18 (владелец):
# брендинг слит с bot.py's whale_monitor() (OI/funding/L-S-ratio институциональный
# скоринг) -- обе фичи теперь показывают пользователю "🐋 WHALE RADAR" (единый
# видимый бренд по спеку Меню v2, раздел РАДАРЫ), но остаются РАЗНЫМИ системами
# под капотом (разные источники данных, разные функции/callback'и) -- не спутать
# при отладке: whale_monitor()/whale_status -- OI/funding; /whales -- этот модуль.
...
def _heartbeat_wrapper(name: str, fn):
    ...
    Владелец, находка 2026-07-15 (критический регресс: bsc_wallet_monitor не
    отмечался 5+ мин при владельце В ПОЗИЦИИ AKE) -- ОТМЕТКА В НАЧАЛЕ ТИКА тоже
    (не только в конце): раньше heartbeat фиксировался ТОЛЬКО после завершения
    fn(), так что если тело задачи зависало (в данном случае -- десятки
    последовательных СИНХРОННЫХ requests.post() внутри async-корутины без
    run_in_executor, блокировавших весь event loop при шторме 1rpc.io rate-limit
    ошибок), watchdog не видел вообще НИЧЕГО, ни успеха, ни ошибки, до победного
    конца зависшего тика -- то есть именно тогда, когда нужнее всего знать, что
    джоб СТАРТОВАЛ, но не может продвинуться. `ok=True` на старте -- это НЕ
    заявление "всё хорошо", это просто "процесс дошёл до этой точки, ещё жив" --
    финальная отметка (успех/исключение) ниже перезаписывает её реальным
    результатом."""
    async def _wrapped(*args, **kwargs):
        _mark_heartbeat(name, ok=True, detail="started")
        try:
...
def whale_monitor_label(direction: str, score_100: float) -> tuple:
    """Ярлык карточки Whale Radar (АПГРЕЙД 11.07 Этап 2.3, переименовано в
    Пакете 18 -- см. коммент выше по файлу про слияние брендинга) -- при скоре <40 карточка
...
def main_kb():
    ...
    return InlineKeyboardMarkup([
        ...
        [InlineKeyboardButton("\U0001f433 Whale Radar",  callback_data="whale_status"),
...
# источника границ zone-touch алертов подписчикам -- WATCHLIST_ZONES больше
# НЕ ведущий источник для алертов, им стал journal/watch_zones.json (тот же
# level_watch.load_watch_zones(), что /zones и /zones_set уже используют).
# Пакет 18, п.1 (владелец): обкатка подтверждена живым логом -- ZONES_UNIFIED
# флаг и LEGACY-ветка (check_watchlist_alerts() на WATCHLIST_ZONES) удалены
# целиком, откат больше не нужен. Сам WATCHLIST_ZONES-словарь НЕ удалён --
# всё ещё кормит SPOT_PORTFOLIO/event_radar_monitor()/`/watchlist`, три
# отдельные живые фичи, не связанные с zone-touch алертами.
...
# -- тот же движок, что /full и /coin (тот же принцип унификации, что применялся к zone-touch алертам выше).
...
async def callback_handler(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "show_menu":
        ...
        try:
            ...
            if found:
                lines_w = [
                    "\U0001f433 *WHALE RADAR — активные сигналы*",
                    f"\U0001f550 _{now_utc3()}_",
                    SEP, "",
                ]
                for w in found[:5]:
...
def _format_whale_alert(w: dict, rug_line: str = "") -> str:
    ...
    lines_out = [
        ...
        f"🐋 *WHALE RADAR — {sym}/USDT*  {dir_emoji}",
        f"🕐 _{now_utc3()}_",
        SEP,
        "",
        # === Блок 2: вердикт ===
...
async def send_scheduled(bot: Bot):
    ...
    try:
        ...
        for coin, want_long in work:
            ...
            # Фаза B Derivatives shadow-continuation, инкремент 2 (владелец,
            # приоритет 1 наряда после инкремента 1): no-op, пока
            # shadow_engine.OPTIONS_AUTO_SHADOW_ENABLED=False (флаг проверяется
            # внутри функции первой строкой, до любого I/O).
            try:
                await shadow_engine.log_auto_options_shadow_async(
                    sym, a, promoted_live=promoted, bot_module=sys.modules[__name__])
            except Exception as e:
                log.error(f"[AUTO] shadow_engine auto_options (не влияет на боевой сигнал) {sym}: {e}")

            # Фаза B Derivatives shadow-continuation, инкремент 4 (владелец,
            # "Наряд на день" 2026-07-15): no-op, пока shadow_engine.
            # LIQUIDATION_AUTO_SHADOW_ENABLED=False (флаг проверяется внутри
            # функции первой строкой, до любого I/O).
            try:
                await shadow_engine.log_auto_liquidation_shadow_async(
                    sym, a, promoted_live=promoted, bot_module=sys.modules[__name__])
            except Exception as e:
...
async def check_watchlist(bot, chat_ids, coins):
    """       5 """
    now_ts   = datetime.now(TZ).timestamp()
    # Пакет 18, п.1 (владелец): LEGACY-ветка (check_watchlist_alerts() на
    # WATCHLIST_ZONES) удалена -- диагностика подтвердила, что живой alert
    # BTC 13.07 20:49 шёл через unified-путь (ZONES_UNIFIED=true на Railway,
    # без него эта ветка вообще не могла выполниться -- см. PROGRESS.md).
    # ZONES_UNIFIED-флаг больше не читается здесь; сам WATCHLIST_ZONES-словарь
    # НЕ удалён -- всё ещё кормит SPOT_PORTFOLIO, event_radar_monitor() и
    # /watchlist, три отдельные живые фичи, не имеющие отношения к
    # zone-touch алертам подписчикам.
    alerts   = check_watchlist_alerts_from_level_watch(coins)
    coin_map = {c["symbol"]: c for c in coins}
    for al in alerts:
        sym = al["symbol"]
...
def get_killzone_status() -> dict:
    ...
    одобрил перенос в бой.

    ОБНОВЛЕНО 2026-07-11, Пакет 4 М3 (владелец, "ДА" -- METHODOLOGY_CORE.md §18.7):
    NY Open расширен до 17:00 (было 14:00-16:00, урок явно даёт "14:00-16:00, иногда
    расширяется до 17:00" -- §8 -- и отдельно "14:00-17:00 после перевода на летнее
    время" -- §18.7, второй независимый прогон того же файла). Лондон Close поднят
    из "не подтверждено источником" в частично подтверждённый -- ДВА источника
    ("5.mp4"/Урок 5 и независимо "37_Стрим_Логика_запуска_алгоритма.mp4") называют
    это окно, НО расходятся в точной границе (17:00-19:00 против 17:00-18:00 Киев,
    конвертированного в 18:00-19:00 UTC+3 Стамбула при зимнем времени Киева) --
    честно беру ШИРЕ (17:00-19:00, объединение обеих оценок), не сужаю точность,
    которой у меня нет. NY Close остаётся не подтверждён -- ни один источник этого
    пакета его не называет.
    ...
    NY Open:          14:00-17:00 UTC+3 (продолжение диапазона, коррекция после Лондона)
    Лондон Close:     17:00-19:00 UTC+3 (два источника, граница между ними расходится -- см. докстринг выше)
    ...
    zones = [
        ...
        {"name": "🇬🇧 Лондон Close",    "start": 17*60, "end": 19*60, "quality": "B",
         "desc": "Закрытие Лондона -- два источника, точная граница расходится (см. докстринг)"},
...
def real_full_analysis(coin: dict) -> dict:
    ...
    # Владелец, 2026-07-18 вечер: ОТКАТ -- гейт оказался контаминирован задвоенным
    # счётом по journal_id (см. ta_extra.smc_setup_type() докстринг, PROGRESS.md
    # "ИСПРАВЛЕНИЕ предыдущей записи"). ta_extra.smc_setup_type() снова wick-only
    # (живая); body_variant ниже -- shadow-кандидат на повторный честный промоушен.
    # Считает ОБА варианта на тех же 4h-свечах (переиспользует c4h_shadow/bias_dir,
    # без новых сетевых вызовов). Не зависит от переменных предыдущего блока (тот
    # блок может упасть независимо -- см. его собственный try/except) --
    # пересчитывает c4h/bias и обе версии здесь же, это чистые функции без сети,
    # повторный вызов дёшев.
    try:
...
async def _startup_integrity_check(bot: Bot, owner_id: int):
    ...
    # Владелец, приёмка v130 (2026-07-16): видимость circuit breaker'а без
    # залезания в логи -- см. cg_rate_limit_status() докстринг.
    cg_status = cg_rate_limit_status()
    if cg_status["in_cooldown"]:
        lines.append(f"🟡 CoinGecko circuit breaker: пауза ещё "
                      f"{cg_status['cooldown_remaining_sec']:.0f}с (подряд 429: "
                      f"{cg_status['consecutive_429']})")
