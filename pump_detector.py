...
Данные для скоринга/алертов — только WS (miniTicker + klines) + CoinGecko/CMC через bot.py.

Двухступенчатая схема:
  1) Грубый детект — один общий стрим !miniTicker@arr (ВСЕ USDT-perp пары разом), rolling-окна
     объёма/цены в памяти на символ. Триггер: Z-Score объёма >3σ + резкое движение цены за 1–5 мин.
  2) На срабатывании — динамическая подписка на kline_1m этого символа (live_prices.request_subscription,
...
# ── Конфигурация: грубый слой (!miniTicker@arr, полное покрытие) ─
MINITICKER_STREAM_URL = "wss://fstream.binance.com/ws/!miniTicker@arr"
...
# ── Состояние грубого слоя (!miniTicker@arr) ─────────────────────
_coarse_price_hist = {}      # symbol -> deque((ts, price))
_coarse_vol_cum = {}         # symbol -> последнее значение кумулятивного 24h quote volume (поле "q")
...
async def run_miniticker_stream(ctx: PumpContext):
    """Общий стрим !miniTicker@arr — грубый детект по ВСЕМ USDT-perp парам разом (полное
    покрытие рынка), без Binance REST. Триггеры уходят в _start_watch().
    ...
    while True:
        try:
            async with websockets.connect(MINITICKER_STREAM_URL, ping_interval=20) as ws:
                print("Pump Radar (coarse): соединение установлено")
                ...
                while True:
                    ...
                    if not first_message_logged:
                        ...
                        if not _startup_notified:
                            ...
                            await _notify_owner(
                                ctx,
                                f"Радар запущен: {len(_coarse_price_hist)} символов, "
                                f"coarse OK, kline {kline_status}")

                    for sym, (kind, price, z, vol_mult) in triggers:
                        await _start_watch(ctx, sym, kind, price, z, vol_mult)
        except Exception as e:
    ...
    while True:
        ...
        try:
            async with websockets.connect(ws_url, ping_interval=20) as ws:
                print("Pump Radar: kline-соединение установлено")
