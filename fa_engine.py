...
def build_full_analysis(symbol: str, coin: dict = None) -> dict:
    ...
    def _oi_matrix():
        ...
        # шум, честное "матрица н/д" ТОЛЬКО в тексте. oi_combo (выше) НЕ меняется --
        # владелец явно попросил боевой скор (_rocket_score() d_oi ниже читает oi_combo)
        # не трогать, только текстовую интерпретацию.
        if oi_change and abs(oi_change) < ta_extra.OI_MATRIX_NEAR_ZERO_PCT:
            oi_text = "OI без изменений — матрица н/д"
