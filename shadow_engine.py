...
def _load_archives() -> list:
    ...
    Владелец, P0 2026-07-21 (живая находка при верификации предыдущего P0-фикса
    -- дедуп не убрал "вне порядка" полностью, 270->0 дублей, но 243 всё ещё
    вне порядка): `sorted(os.listdir(...))` -- АЛФАВИТНАЯ (строковая), не
    числовая сортировка числового суффикса `_unique_archive_path()`
    (`shadow_signals_<дата>_<дата>_N.json`) -- "_123.json" лексикографически
    МЕНЬШЕ "_13.json" (сравнение по символам: '2' < '3' на второй позиции),
    хотя число 123 > 13. При частой ротации в пределах одних суток (что и
    происходит с сегодняшним count-based порогом) число файлов с одинаковым
    датовым диапазоном легко переваливает за 10, суффиксы достигают двух-трёх
    цифр -- строковая сортировка тасует их не по времени создания, что и
    давало ложные "вне порядка" при склейке РАЗНЫХ (не одного файла) архивов
    одной и той же датой. Фикс -- НЕ полагаться на порядок файлов вообще:
    сортируем итоговый список записей ПО ts ПОСЛЕ сборки из всех файлов --
    гарантирует хронологический порядок независимо от того, как называются/
    сортируются сами файлы. Записи без ts (битая схема) -- в конец списка
    (не мешают сортировке того, что честно сортируемо)."""
    records = []
    ...
    records.sort(key=lambda r: (r.get("ts") is None, r.get("ts") or 0))
    return records
...
def _sync_to_github_sync(new_record: dict = None, now: float = None) -> bool:
    ...
    PUT'ит локальное состояние, если есть расхождение (новые записи ЛИБО
    записи, ушедшие из активного файла в архив ротацией -- см. ниже). Best-
    effort -- отсутствие GITHUB_TOKEN просто пропускает шаг, это не ошибка.

    Владелец, P0 2026-07-21 (живая находка при верификации ЭТОГО ЖЕ пакета,
    ДО того, как стало проблемой на практике): remote-копия ДОЛЖНА зеркалить
    ТЕКУЩЕЕ содержимое активного файла, а не расти монотонно. Раньше пуш
    отправлял `remote_records + missing` (remote плюс новые локальные) --
    корректно при старой (2026-07-14..07-20) time-based ротации, где активный
    файл усыхал редко и remote просто "довозил хвост". С сегодняшним
    count-based порогом (ROTATION_MAX_ACTIVE_RECORDS=1000) ротация происходит
    ЧАСТО -- если remote продолжать только ДОПОЛНЯТЬ, а не зеркалить, он бы
    рос НЕОГРАНИЧЕННО (тот же класс проблемы, из-за которой затевался весь
    этот P0-пакет, просто на уровне git-истории вместо REST-payload'а).
    Правильное поведение: remote = точная копия локального активного файла
    -- пропускаем сеть только если множества uid ЛОКАЛЬНОГО и REMOTE
    совпадают ТОЧНО (не "локальных нет вне remote", а "оба множества равны"
    -- иначе пропустили бы случай "remote содержит лишнее, локально уже
    ротировано", и remote никогда бы не усох).

    Ретрай-catchup (найдено 2026-07-12/13, см. SHADOW_ANALYSIS.md запись 23:42):
    пуш локального состояния (не только `new_record`) сам по себе уже решает
    старую находку "предыдущая попытка не синканула свою запись" -- следующий
    успешный синк всегда берёт ПОЛНОЕ текущее содержимое активного файла.

    Также НЕ трактует ошибку GET как пустой файл (см. _github_get_shadow_sync) --
    транзиентный сбой прерывает синк без попытки PUT, чтобы не рисковать
    перезаписью существующих записей меньшим (локальным) списком.
...
def _record_uid(rec: dict) -> str:
    """uid = hash(symbol + contour + ts + type) -- владелец, приёмка v130
    (2026-07-16, регресс к #245): формула расширена до 4 полей (контур
    добавлен) по прямому наряду владельца -- предыдущая версия (3 поля,
    без contour) молча склеивала бы в один uid два РАЗНЫХ контура,
    случайно совпавших по symbol+type+ts (напр. tz13 и external_trader1_btc
    на одном тике одного символа), тихо теряя одну из двух легитимных
    записей на этапе _write_local() без единой ошибки в логе."""
    raw = f"{rec.get('symbol', '')}|{rec.get('contour', '')}|{rec.get('ts', '')}|{rec.get('type', '')}"
    return hashlib.sha256(raw.encode()).hexdigest()


def _warm_uid_index() -> set:
    """Владелец, приёмка v130 (2026-07-16, регресс к #245): прогрев ТЕПЕРЬ
    включает архив (get_local_records(include_archive=True)), не только
    активный файл. Живая находка: диагноз #245 ("дубли живут исключительно
    в архиве, активный писатель уже идемпотентен") оказался НЕПОЛНЫМ --
    прогрев из одного активного файла НЕ ловит случай, когда запись с uid,
    уже когда-то ушедшим в архив через ротацию (_rotate_if_needed), позже
    ЗАНОВО попадает на запись (напр. если вызывающий код переигрывает уже
    обработанный исторический бар после рестарта процесса, до того как его
    прогресс-бар/bookmark был сохранён) -- индекс, прогретый только из
    активного файла, никогда не видел этот uid и не может её отклонить,
    что и породило постоянные active<->archive дубли, найденные владельцем
    живьём (997->1042). Архив на порядок меньше активного файла после
    санации #245 (~1K записей против ~5-6K) -- цена скана на старте/после
    ротации приемлема, это НЕ горячий путь записи."""
