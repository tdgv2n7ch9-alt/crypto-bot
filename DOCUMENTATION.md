...
Составлено 2026-07-10, обновлено 2026-07-11 (пакет «АПГРЕЙД 11.07», Этапы 1-5 —
новые модули whale_radar.py/level_watch.py/daily_metrics.py/backtest/journal_backfill.py,
честность витрины, ФАЗА B деривативов, run_tracker-фикс) по факту текущего кода на `main`
(не по памяти/намерениям — каждая строка ниже сверена с реальным файлом/строкой при
написании). Это карта "что есть и как устроено", не архитектурное предложение — изменений
в этом документе быть не должно без изменений в самом коде.

## 1. Модули и их роли

| Модуль | Строк | Роль |
|---|---|---|
| `bot.py` | ~10940 | Точка входа. Telegram-хендлеры (команды/кнопки), APScheduler (все job'ы), HTTP-обёртки к CoinGecko/CMC/Bybit/Yahoo с кэшем и rate-limit, health/watchdog/integrity-check, `/stats`/`/journal`, сборка карточек сигналов. |
...
| `signal_journal.py` | ~906 | Paper-trading трекер всех сигналов (см. §3, схема журнала). GitHub Contents API как персистентность поверх Railway-эфемерного диска. |
...
| `whale_radar.py` | ~644 | Whale Radar — Bybit WS (`orderbook.200.*`/`publicTrade.*`) → крупные лимитки/сделки в зоны + CVD (Этап 3.1 АПГРЕЙД 11.07, тот же WS-поток). `/whales`, owner-алерт ≥$200K, chart_v4 whale-слой (owner-only). |
| `level_watch.py` | ~378 | Аддитивный вотчер РУЧНОЙ дневной разметки владельца (`journal/watch_zones.json`, реплейс-семантика с историей). Алерты приближения/касания зоны, `/zones`, `/zones_set`. Логирует касания в JSONL (Этап 4) для «Метрики дня». |
| `daily_metrics.py` | ~194 | «Метрики дня» (Этап 4 АПГРЕЙД 11.07) — ежедневная сводка owner-чату в 21:00 TZ бота: сигналы (создано/TP/SL), shadow vs live, whale-события топ-3, касания level-watch зон, здоровье источников. Только чтение существующих источников. |
| `backtest/journal_replay.py` | ~217 | Только чтение `signal_journal.get_closed_records()` → win-rate/avg-R/max-drawdown по source и по режиму рынка. Не влияет на сигнальный цикл. |
| `backtest/journal_backfill.py` | ~296 | Разовый скрипт (не боевой цикл): реплей PENDING/ENTERED/EXPIRED-записей `journal/signals.json` реальными часовыми свечами Bybit — задним числом досчитывает исходы, которые живой `run_tracker()` пропустил (нет WS-цены по символу). Меняет только статусные поля, ID не трогает. |
...
`live_prices`, наблюдение не влияет на сигналы. `live_prices.get_live_price(symbol)`
возвращает цену только по символам, которые хоть раз подхватило WS-покрытие
`pump_detector.py` (топ-20 + динамические запросы) — по остальным символам исход не
определится вовсе, пока не появится хоть один тик. **С 2026-07-11**: 72ч-истечение
PENDING больше НЕ зависит от наличия цены (раньше зависело — символ без WS-покрытия
не только не получал исход, но даже не истекал по таймеру, см. `PROGRESS.md`, находка
из `backtest/journal_backfill.py`) — таймер работает всегда, вход/SL/TP всё ещё
требуют цены явно. Персистентность: локальный JSON + GitHub Contents API
(`journal/signals.json`), last-write-wins по `updated_ts`, батчинг коммитов раз в 5
мин. Ротация: записи закрытые >180 дней уходят в годовые архивные файлы
(`_rotate_old_records()`, раз в час). Ежедневный версионированный бэкап —
...
`backtest/journal_backfill.py` (реплей часовыми свечами Bybit, разово, не боевой
цикл — досчитывает исходы, которые пропустил живой трекер из-за отсутствия WS-цены),
`daily_metrics.py` (ежедневная сводка), `tests/test_journal_historical_examples.py`.
...
- **`degraded_data`-флаг** (ROADMAP П3) — не блокирует сигналы, только помечает для
  ...
  выборка.
- **Whale/level-watch событийные логи — ЛОКАЛЬНЫЙ диск, не GitHub** (`whale_radar.
  EVENTS_DIR`/`level_watch.EVENTS_DIR`, JSONL-по-дате) — та же эфемерность диска
  Railway, что была решена для журнала/подписчиков GitHub-персистентностью, здесь
  сознательно НЕ решена (низкая ценность истории дольше одного дня не оправдывает
  ещё один GitHub Contents API поток). Практическое следствие: `daily_metrics.py`
  («Метрики дня») видит эти события только с момента последнего рестарта процесса,
  не всегда с полуночи — честно указано прямо в тексте самой сводки.
- **CVD (`whale_radar.WhaleRadarState.cvd_windows`) — только in-memory, только
  текущий процесс.** Копится с момента последнего рестарта того же процесса, что
  ведёт Whale Radar/orderbook-сканирование — недоступен ни из отдельного скрипта,
  ни после редеплоя без повторного накопления. Не персистентен, аналогично
  событийным логам выше.

## 7. Как читать этот документ

Карта отражает код на момент 2026-07-10 (после git-реконсиляции 14→134 коммитов и
всех П1-П3 правок этой сессии), обновлена 2026-07-11 после пакета «АПГРЕЙД 11.07»
(Этапы 1-5: честность карточек, ФАЗА B первых деривативов, «Метрики дня»,
run_tracker-фикс, ретроспективный пересчёт журнала — см. `PROGRESS.md` за детали
и живые числа по каждому пункту). Не переписывать вручную при мелких правках — при
